class CrafesController < ApplicationController
  ...
  def index
    ...
    contests.each do |contest|
      if contest.start_datetime <= Time.now && contest.start_datetime + contest.duration * 60 >= Time.now
        @in_session_contests << contest
      elsif contest.start_datetime > Time.now
        @scheduled_contests << contest
      end
    end
